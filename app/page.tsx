...
export default function Home() {
  ...
  return <div className={result ? "shell shell-report" : "shell"}><AssessmentHeader mode={mode} />{result ? <main className="assessment-workspace">{error && <div className="error" role="alert">{error}</div>}<AssessmentDashboard result={result} onCheckAnother={() => { setResult(undefined); setError(undefined); }} /></main> : <main className="start-screen"><section className="start-copy"><div className="eyebrow">COMMERCIAL EV INFRASTRUCTURE</div><h1>Know the site<br />before the truck rolls.</h1><p>InstallIQ verifies location, preserves live data provenance, and prepares the human survey that follows.</p><div className="start-points"><span>Verified location</span><span>Operating-area check</span><span>Field-survey handoff</span></div></section><InstallRequestForm onRun={run} running={running} />{error && <div className="error" role="alert">{error}</div>}</main>}</div>;
}
